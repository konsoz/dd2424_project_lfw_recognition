...
def build_image_dataset_from_dir(directory,
    ...
    try:
        ...
        if shuffle_data:
            X, Y = shuffle(X, Y)
        pickle.dump((X, Y), open(dataset_file, 'wb'), 2)
    return X, Y
...
def directory_to_samples(directory, num_img_threshold=10, flags=None, filter_channel=False):
    ...
    for c, dir_name in enumerate( classes ):
        c_dir = os.path.join(directory, dir_name)
        try: # Python 2
            walk = os.walk(c_dir).next()
        except Exception: # Python 3
            walk = os.walk(c_dir).__next__()

        num_instances = len(walk[2])
        if(num_instances > num_img_threshold):
