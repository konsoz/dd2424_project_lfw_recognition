...
def load_data2(dirname="face", resize_pics=(224, 224), shuffle=True,
    one_hot=False):
    """
    http://vis-www.cs.umass.edu/lfw/lfw-deepfunneled.tgz
    
    """
    dataset_file = os.path.join(dirname, 'lfw-deepfunneled.pkl')
    if not os.path.exists(dataset_file):
        tarpath = maybe_download("lfw-deepfunneled.tgz",
                                 "http://vis-www.cs.umass.edu/lfw/",
                                 dirname)

    X, Y = build_image_dataset_from_dir(os.path.join(dirname, 'lfw-deepfunneled/'),
                                        dataset_file=dataset_file,
                                        resize=resize_pics,
                                        filetypes=['.jpg', '.jpeg'],
                                        convert_gray=False,
